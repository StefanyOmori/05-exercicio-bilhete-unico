...
public class Util {
    ...
    private int pesquisar() {
        int posicao = -1; //posição que não existe no vetor, exceto em python
    ...
    private void removerBilhete(){
        int posicao = pesquisar();
        if (posicao != -1){
            int resposta = showConfirmDialog(null, "Tem certeza que deseja remover o bilhete?");
            if(resposta==YES_OPTION){
                bilhete[posicao]=bilhete[index-1]; //index = topo
                index--;
            }
        }
    }
